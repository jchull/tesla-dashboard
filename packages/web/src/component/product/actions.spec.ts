...
describe('product actions', () => {
  it('should handle empty list', async () => {
    services.queryService.getProducts = jest.fn(
      (): Promise<types.Vehicle[]> => {
        return new Promise<types.Vehicle[]>((resolve, reject) => resolve([]))
      }
    )
  ...
  it('should handle simple product list', async () => {
    services.queryService.getProducts = jest.fn(
      (): Promise<types.Vehicle[]> => {
        return new Promise<types.Vehicle[]>((resolve, reject) =>
          resolve([sampleCar])
        )
      }
    )
