...
export const ChargeListItemDetail: React.FC<SessionListItemState> = (
  props
) => {
  const item = props.session
  const durationMinutes = moment
    .duration(moment(item.end_date)
      .diff(moment(item.start_date)))
    .asMinutes()
  const last = item.last || item.first
  const rangeAdded = last.charge_state.charge_miles_added_rated
  ...
  const estChargingCost = last.charge_state.charge_energy_added * costPerKwh
  ...
  return (
    <div>
      <div className="row">
        <div className="start">
          <span>
            {last._id !== item.first._id ? last.charge_state.charge_energy_added : 0} kWh
          </span>
        </div>
        <span>
          {last._id !== item.first._id ? last.charge_state.charge_miles_added_rated : 0}{' '}
          miles
        </span>
        <div className="end">
