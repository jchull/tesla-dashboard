...
export class ProductService {
  ...
  async findRecentSessions(username: string, productId: string, limit: number) {
    ...
    const driveSessions = await this.driveSessionModel.find({vehicle})
                                            .sort({$natural: -1})
                                            .limit(limit)
                                            .populate(['first', 'last', 'vehicle']);
    const chargeSessions = await this.chargeSessionModel.find({vehicle})
                                              .sort({$natural: -1})
                                              .limit(limit)
                                              .populate(['first', 'last', 'vehicle']);
    const sessions = [...driveSessions, ...chargeSessions]
        .sort((a: {start_date: number}, b: {start_date: number}) => b.start_date - a.start_date)// reverse sort
        .slice(0, limit);
    if (sessions.length) {
      return sessions;
    }
