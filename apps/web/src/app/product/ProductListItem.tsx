...
import { BatteryLevelIcon } from '@tesla-dashboard/components';
...
export const ProductListItem: React.FC<ProductListItemState> = (props: ProductListItemState) => {
  ...
  return (
    <div className="list-item product red_pinwheel18" onClick={() => props.handleSelection(props.product._id)}>
      <div className="row">
      ...
      <div className="row">
        <div className="odometer start">{numbro(props.product.odometer).format('0,0.0')} miles</div>
      </div>

      <div className="row">
        <div className="status start">
          {`${props.product.state[0]}${props.product.state.substr(1).toLowerCase()} `}
