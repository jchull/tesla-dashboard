...
import { TeslaTokenComponent } from './TeslaTokenUpdater';
...
export const TeslaAccountComponent: FC<TeslaAccountProps> = (props: TeslaAccountProps) => {
  ...
  function handleChange(event: ChangeEvent<HTMLInputElement>) {
    const { _id, username, email, access_token, refresh_token } = Object.assign({}, account, {
  ...
  useEffect(() => {
    const fv = (account && account.email && emailRegex.test(account.email)) || false;
    setFormValid(fv);
  }, [account.email]);
  ...
  return (
    <div className="centered narrow">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input placeholder="Email" name="email" type="email" value={account.email} onChange={handleChange} />
        <label htmlFor="token">Token</label>
        <input placeholder="Token" name="token" type="password" value={account.access_token} onChange={handleChange} />
        ...
        <button onClick={validateToken} disabled={!account.email || !account._id || !account.access_token}>
          Validate Token
        </button>
