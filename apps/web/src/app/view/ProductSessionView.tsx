...
export const ProductSessionView: React.FC = () => {
  ...
  React.useEffect(() => {
    if (selectedProduct) {
      dispatch(fetchSessionListAction(selectedProduct._id, { start: 0, size: 100 }));
    }
  }, [selectedProduct]);
  ...
  const onResize = () => {
    ...
    if ((chartOptions.height !== calcHeight || chartOptions.width !== calcWidth) && Date.now() - lastResize > 1000) {
  ...
  return (
    <div className="columns three">
      ...
      <div ref={mainContent}>
        <LineChart datum={selectedSessionStates} options={chartOptions} />
        <SessionTagList />
        <CommandPanel />
      </div>
      <div>
