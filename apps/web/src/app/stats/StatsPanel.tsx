...
export const StatsPanel: React.FC = () => {
  ...
  const loadedCount = useSelector((store: AppState) => store.session.loadedCount);
  ...
  const currentRangeMilesUsed =
    (selectedSession?.first?.battery_range ?? 0) - (selectedSession?.last?.battery_range ?? 0);
    ...
    <div className="card">
      ...
      {selectedSession?.activity === ActivityType.DRIVING ? (
        <>
          <section>
            <h6>Current</h6>
            <div>Distance: {numbro(currentDistance ?? 0).format(NUMBER_FORMAT_DEC)} mi</div>
            <div>Range Used: {numbro(currentRangeMilesUsed ?? 0).format(NUMBER_FORMAT_DEC)} mi</div>
          </section>
        </>
      ) : (
        <>
          <section>
            <h6>Current</h6>
            <div>Range Added: {numbro(0).format(NUMBER_FORMAT_DEC)} mi</div>
          </section>
        </>
      )}
      <section>
        ...
        {selectedSession?.activity === ActivityType.DRIVING ? (
          <>
            <div>Distance: {numbro(totalMiles ?? 0).format(NUMBER_FORMAT_DEC)} mi</div>
            <div>Range Used: {numbro(allRangeMilesUsed ?? 0).format(NUMBER_FORMAT_DEC)} mi</div>
          </>
        ) : (
          <>
            <div>Range Added: {numbro(allRangeMilesUsed * -1.0).format(NUMBER_FORMAT_DEC)} mi</div>
            <div>Energy Added: {numbro(0).format(NUMBER_FORMAT_DEC)} kWh</div>
          </>
        )}
      </section>
      <div className="card-footer">
        <div>{loading ? 'Loading...' : `Showing ${loadedCount ?? 0} of ${totalCount ?? 'unknown'}`}</div>
      </div>
    </div>
  ) : (
