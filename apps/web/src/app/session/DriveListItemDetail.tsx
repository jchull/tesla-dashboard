...
export const DriveListItemDetail: React.FC<SessionListItemState> = (props) => {
  const durationHours = moment
    .duration(moment(props.session.end_date).diff(moment(props.session.start_date)))
    .asHours();
  // const rangeUsed = props.session.first.battery_range - props.session.last.battery_range;

  const distanceTraveled = props.session.last ? props.session.distance : 0;
      ...
      {distanceTraveled > 0 ? (
        <div>
          <div className="row">
            ...
            <div className="end">{numbro(averageSpeed).format('0,0.0')} mph</div>
          </div>
          <div className="row">
            <div className="start">
