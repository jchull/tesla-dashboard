...
export class TeslaOwnerService {
  ...
  async updateToken(
    ...
    const updatedAccount = await this.teslaAccountService.updateAccount(
      response
    )
  ...
  async getVehicleData(
    ...
    return (
      ...
      fetchVehicleData(this.config, teslaAccount, id_s)
    )
  }
  ...
  async getNearbyChargers(teslaAccount: TeslaAccount, id_s: string) {
    return (
      (await this.checkToken(teslaAccount)) &&
      fetchNearbyChargers(this.config, teslaAccount, id_s)
    )
  }
  ...
  async getVehicle(teslaAccount: TeslaAccount, id_s: string) {
    return (
      (await this.checkToken(teslaAccount)) &&
      fetchVehicle(this.config, teslaAccount, id_s)
    )
  }
}
