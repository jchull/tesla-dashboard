...
export const fetchToken = async (
  ...
  const data = {
    ...
    grant_type,
    [grant_type]: password?.length && grant_type === 'password' ? password : teslaAccount[grant_type],
  };
...
export const fetchVehicles = async (config: Configuration, teslaAccount: TeslaAccount): Promise<Vehicle[]> => {
...
export const fetchNearbyChargers = async (config: Configuration, teslaAccount: TeslaAccount, id_s: string) => {
  const response = await axios.get(
...
export const fetchVehicle = async (config: Configuration, teslaAccount: TeslaAccount, id_s: string) => {
  const response = await axios.get(
