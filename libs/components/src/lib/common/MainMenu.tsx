...
export const MainMenu: React.FC = () => {
  ...
  const username = useSelector((store: { auth: { username: string } }) => store.auth.username);
  ...
  const content = username ? mapItemsToElements(loggedInMenuItems) : mapItemsToElements(loggedOutMenuItems);
  ...
  return (
    <div className="main-menu" onClick={() => setShowing(!showing)}>
      ...
      <div className={showing ? 'menu-items showing' : 'menu-items'}>{content}</div>
    </div>
  );
};
