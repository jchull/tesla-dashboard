...
it('renders empty tag list', () => {
  wrapper = render(<TagList tags={[]} addListener={console.log} removeListener={console.log} />);
  expect(pretty(wrapper.container.innerHTML)).toMatchSnapshot();
});

it('renders simple tag list', () => {
  wrapper = render(<TagList tags={['item_1', 'item_2']} addListener={console.log} removeListener={console.log} />);
  expect(pretty(wrapper.container.innerHTML)).toMatchSnapshot();
});
...
xit('adds tag from input', async () => {
  ...
  wrapper = render(<TagList tags={[]} addListener={addListener} removeListener={console.log} />);
  ...
  expect(pretty(wrapper.asFragment())).toMatchSnapshot();
});
